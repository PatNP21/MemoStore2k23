...
app.post('/addNote', async (req, res) => {
    const {user_id, title, content, multimedia} = req.body
    try {
        ...
        console.log({user_id, title, content, multimedia})
...
app.delete('/deleteItem/:id', async (req, res) => {
    ...
    post_collection.deleteOne({"_id": id}).then((data) => {
        res.status(200).json(data)
    }).catch(err => {
        console.log(err)
        res.status(500).json({'result': 'error occured!'})
...
app.get('/getImage', async (req, res) => {
    let cloud = new Cloudinary(req.body, 'user_folder')
